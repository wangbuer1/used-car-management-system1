...
import com.sx.dao.*;
import com.sx.pojo.*;
import com.sx.pojo.Vo.HistoryVo;
...
//        session.setAttribute("user",id);
    ...
    public ModelAndView showDetails(HttpSession session, Integer id, HttpServletRequest request) throws Exception {
        //封装carInfo和pic_url
        ...
        request.setAttribute("islocked",carInfo.getCarInfo().getIslocked());
...
//        System.out.println("user:"+user_id);
    ...
    public ModelAndView checkCar(HttpSession session){
        ...
        List<CarInfo> itemsList = new ArrayList<CarInfo>();
        for (CarInfo carInfo:itemsListTempt) {
            if(carInfo.getIslocked().equals("可出售")||carInfo.getIslocked().equals("不可出售")||carInfo.getIslocked().equals("待审核"))
                itemsList.add(carInfo);
        }
